<?php
/**
 * @return an array of callable methods (NOT include inherited methods)
 */
...
/**
 * @return an array of superclasses (including the current class)
 */
...
function print_object_info( $object ) {
    ...
    array_pop($lineage); // remove the current object
    echo count( $lineage ) ? ( '<p>' . join( ' -&gt ', $lineage) . '</p>' )
                        : ( '<p>' . '<i>None</i>' . '</p>' );
                        
    echo '<h3>Children</h3>';
    $children = get_child_classes( $object );
    echo count( $children ) ? ( '<p>' . join( ', ', $children) . '</p>' )
                        : ( '<p>' . '<i>None</i>' . '</p>' );
    ...
    if( !count($methods) ) {
        ...
        foreach ($methods as $method ) {
            echo in_array( $method , (array) $object_methods ) ? "<b>$method</b>( );<br>" : "<i>$method</i>( );<br>";
        }
    }
